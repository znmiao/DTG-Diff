...
class TaskGradientGuidance:
    ...
    def influence_gradient(self, x: torch.Tensor, target: torch.Tensor,
        ...
        failure = failure_grad.to(g_vec.device)
        if self.config.preconditioner == "cosine":
            denom = failure.norm().detach() * g_vec.norm().detach() + self.config.grad_eps
        elif self.config.preconditioner == "none":
            denom = torch.ones((), device=g_vec.device)
        else:
            denom = g_vec.pow(2).sum().detach() + self.config.grad_eps
        align = torch.dot(failure, g_vec) / denom
