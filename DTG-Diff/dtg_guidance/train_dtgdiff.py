...
It expects arrays in shape [N, C, T]. Use --data_path to load .npz files with keys:
...
def main():
    ...
    few = torch.as_tensor(few).float()
    mask = torch.as_tensor(mask).float() if mask is not None else None
